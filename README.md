...
启用 AI 后，你也可以用「一笺成文」把当天碎片记录整理成一篇第一人称日记。整理好的日记保存在本机，可在日记窗口查看、编辑、删除，并按需导出 PDF。
...
- 日记窗口查看、编辑、删除整理好的日记。
- 「一笺成文」把当天记录整理成第一人称日记。
- 可配置 OpenAI 兼容接口的 `base_url`、`model` 和 API Key。
- API Key 存入 Keychain，不写入本地 JSON。
- 不做云同步。
- 基础记录功能不需要人工智能密钥。
- 不自动上传任何记录；AI 只在用户点击「一笺成文」后调用。
...
- 记录只保存在本机 JSON 数据文件里。
- 整理好的日记也只保存在本机 JSON 数据文件里。
- PDF 只保存到你选择的目录。
- 应用不会自动上传记录，也不会在后台调用人工智能服务分析你的数据。
- 如果你点击「一笺成文」，应用会把当天记录直接发送给你配置的第三方模型服务，用来生成第一人称日记。
- AI API Key 保存在 Keychain，不写入本地 JSON。
- 我们的承诺是：没有自有服务器，不收集、不保存、不转发你的记录与日记结果。
